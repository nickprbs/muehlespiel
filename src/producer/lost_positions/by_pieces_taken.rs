    ...
    for mill_board in with_just_mills_hash.iter() {
        ...
        _all_other_stones_iter.for_each(|additional_winner_stones| {
            ...
            let new_board: GameBoard = [
                temp_gameboard[0] | mill_board[0],
                temp_gameboard[1] | mill_board[1],
    ...
    for _two_stone_board in two_stones_hash.iter() {
        ...
        while let Some(single_mill_position) = mill_iter.next() {
            if is_canonical_free_at(*_two_stone_board, single_mill_position[0]) &&
                ...
                let new_board: GameBoard = [
                    _two_stone_board[0] | mill_board[0],
                    _two_stone_board[1] | mill_board[1],
...
#[test]
pub fn test_number_of_lost_positions() {
    let count = lost_positions_by_pieces_taken().len();
    assert!(count == 7729592 || count == 7825361);
}